...
animals::Sheep::Sheep(World &organism_world, Container *world_organisms, Organism* **organisms_array, bool born, int parent_pos_x, int parent_pos_y) {
	Random randomNumber;
	myException notBorn;
	int posX, posY;
	...
	if (born == false) { // Losowanie pozycji i sprawdzanie czy nie jest juz zajeta dla nie urodzonego organizmu
		while (true) {
			...
			if (organisms_array[posX][posY] == nullptr) {
				Organism::setOrganism(4, 4, 1, "Sheep", posX, posY, organism_world);
...
animals::Sheep::Sheep(World &organism_world, Container *world_organisms, Organism* **organisms_array, int strength, int initiative, int age, string name, int posX, int posY) {
...
int animals::Sheep::reproduce(Container *world_organisms, Organism* **organisms_array, int parentPosX, int parentPosY) {
